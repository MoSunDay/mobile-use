...
export class Executor {
  ...
  constructor(
    ...
    const navigatorActionRegistry = new NavigatorActionRegistry(
      actionBuilder.buildAppiumActions()
  ...
  async execute(): Promise<void> {
    ...
    try {
      ...
      for (step = 0; step < allowedMaxSteps; step++) {
        ...
        if (
          this.planner &&
          (context.nSteps % context.options.planningInterval === 0 || validatorFailed)
        ) {
          validatorFailed = false;
          // The first planning step is special, we don't want to add the device state message to memory
          ...
          if (planOutput.result) {
            ...
            if (!planOutput.result.app_task && planOutput.result.done) {
              break;
            }
          }
        }
        ...
        if (
          ...
          // TODO: add a stopping condition to avoid loop of planner<>validator
        }
      }
