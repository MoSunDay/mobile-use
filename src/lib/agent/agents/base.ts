...
import { AgentContext, AgentOutput, AgentOptions, agentBrainSchema } from '../types';
...
// Define our own BaseAgentOptions interface
...
export abstract class BaseAgent<T extends z.ZodType, M = unknown> {
  ...
  async invoke(inputMessages: BaseMessage[]): Promise<this['ModelOutput']> {
    ...
    if (this.withStructuredOutput) {
      ...
      try {
        // logger.info('LLM Request message:', inputMessages[inputMessages.length - 1].content);
        const response = await structuredLlm.invoke(inputMessages, {
          ...this.callOptions,
        });
        // logger.info('LLM Response:', JSON.stringify(response.parsed, null, 2));
        if (response.parsed) {
