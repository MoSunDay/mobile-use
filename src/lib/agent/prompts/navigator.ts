...
2. ACTIONS: You can specify multiple actions in the list to be executed in sequence. But always specify only one action name per item.
   ...
   - Navigation: [
       {"open_app": {"app": "com.android.chrome"}},
       {"go_to_app": {"app": "com.android.chrome"}},
     ]
...
4. NAVIGATION & ERROR HANDLING:
   ...
   - Handle popups of privacy/cookies/tracking/gdpr by accepting or closing them
...
`;
  ...
  inputFormat(): string {
    return `
INPUT STRUCTURE:
1. Current App: The app you're currently on
2. Available Apps: List of available apps
3. Interactive Elements: List in the format:
   index[:] element (attributes and values)
   - index: Numeric identifier for interaction
   - element: Android View hierarchy element type (VIEW, TEXTVIEW, VIEWGROUP, etc.)
   - attributes and values: Attributes and values of the element eg: text, id, bounds, accessibilityText, clickable, enabled, etc.

Example:
  5: ViewGroup(bounds=[64,297][1016,756], clickable=true)
  6: ViewGroup(bounds=[64,297][1016,756])
  7: TextView(text=Welcome, id=typography, bounds=[127,360][953,421])
  8: TextView(text=We look forward to your feedback. Enjoy the experience, id=typography, bounds=[127,445][540,627])

Notes:
- Only elements with clickable=true are interactive
- some elements provide context but cannot be interacted with
`;
  }
  ...
  getSystemMessage(): SystemMessage {
     ...
     */
    const AGENT_PROMPT = `You are a precise android automation agent that interacts with apps through structured commands. Your role is to:
1. Analyze the provided page elements and structure
2. Use the given information to accomplish the ultimate task
3. Respond with valid JSON containing your next action sequence and state assessment
4. If the page is asking for login credentials, never try to fill it by yourself. Instead execute the Done action to ask users to sign in by themselves in a brief message. Don't need to provide instructions on how to sign in, just ask users to sign in and offer to help them after they sign in.
...
Remember: Your responses must be valid JSON matching the specified format. Each action in the sequence must be valid.`;
  ...
  async getUserMessage(context: AgentContext): Promise<HumanMessage> {
    return await this.buildMobileStateUserMessage(context);
  }
}
